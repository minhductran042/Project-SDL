...
bool MainCharacter::LoadImg(std::string path, SDL_Renderer* screen)
{
	...
	if (ret == true)
	{
		width_frame_ = rect_.w / PLAYER_NUM_FRAME;
...
void MainCharacter::HandleInputAction(SDL_Event events, SDL_Renderer* screen, Mix_Chunk* bullet_sound[2])
{
...
void MainCharacter::CharacterAction(Map& map_data, Mix_Chunk* sound_player[2])
{
	if (come_back_time_ == 0)
	{
		x_val_ = 0;
...
void MainCharacter::CheckCrashingEvents(Map& map_data)
{
	int x1 = 0;
	int x2 = 0;
	int y1 = 0;
...
void MainCharacter::IncreaseEnergy()
{
	energy_amount++;
