...
void MainObject::set_clip() {
	if (width_frame_ > 0 && height_frame_ > 0) {
		...
		frame_clip_[1].x = 1*width_frame_;
		frame_clip_[1].y = 0;
...
void MainObject::Show(SDL_Renderer* des) {
	...
	// tao kich thuoc chuan
	SDL_RenderCopy(des, p_object_, current_clip_, &renderQuad);
	// day p_object len man hinh, current_clip_ lien tuc thay doi nen tao ra hinh anh dong
}
...
void MainObject::DoPlayer(Map& map_data)
{
	...
	if (come_back_time > 0) {
		come_back_time--;
		if (come_back_time == 0) { //reset tai trang thai
			on_ground_ = false;
			if (x_pos_ > 256) {//256=4 o tile map
				x_pos_ -= 256;
			}
			else {
				x_pos_ = 0;
...
void MainObject::CenterEntityOnMap(Map& map_data) {
	...
	if (map_data.start_y_ < 0) {
		...
		map_data.start_y_ = map_data.max_y_ - SCREEN_WIDTH;
	}
}
...
void MainObject::CheckToMap(Map& map_data) {
	...
	if (x1 >= 0 && x2 <= MAX_MAP_X_ && y2 >= 0 && y2 <= MAX_MAP_Y_) {
		if (y_val_ > 0) {
