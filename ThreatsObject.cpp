...
void ThreatsObject::ThreatsAction(Map& gMap)
{
	if (come_back_time_ == 0)
	{
		x_val_ = 0;
...
void ThreatsObject::CheckCrashingEvent(Map& map_data)
{
	int x1 = 0;
	int x2 = 0;
	int y1 = 0;
	...
	if (x1 >= 0 && x2 < MAX_MAP_X && y1 >= 0 && y2 < MAX_MAP_Y)
	{
		if (x_val_ > 0) // main object is moving to right
		{
			...
			if ((val1 != BLANK_TILE && val1 != ENERGY) || (val2 != BLANK_TILE && val2 != ENERGY))
			{
				x_pos_ = x2 * TILE_SIZE;
